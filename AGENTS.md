...
- `adr/` - Architecture Decision Records; short rationale for standing conventions. Read before challenging one. New
  ADRs must follow `adr/0000-template.md`; keep its section order and leave Status last.
...
- Labels are not decoration - release notes are generated from them: one type label
  ...
  get `DX` not UX - and `IDX` for Infection developer experience) plus component
