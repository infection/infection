...
final class ProtectedVisibility implements Mutator
{
    ...
    {
        return new Definition('Replaces the `protected` method visibility keyword with `private`.', MutatorCategory::SEMANTIC_REDUCTION, null, <<<'DIFF'
...
DIFF
    ...
    */
    ...
    {
        (yield new Node\Stmt\ClassMethod($node->name, ['flags' => $node->flags & ~Node\Stmt\Class_::MODIFIER_PROTECTED | Node\Stmt\Class_::MODIFIER_PRIVATE, 'byRef' => $node->returnsByRef(), 'params' => $node->getParams(), 'returnType' => $node->getReturnType(), 'stmts' => $node->getStmts()], $node->getAttributes()));
    }
