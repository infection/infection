...
use _HumbugBox9658796bb9f0\Infection\Mutator\FunctionSignature\ProtectedVisibility;
use _HumbugBox9658796bb9f0\Infection\Mutator\FunctionSignature\PublicVisibility;
...
use _HumbugBox9658796bb9f0\Infection\Mutator\Removal\MethodCallRemoval;
...
{
    private const DETECTION_STATUS_MAP = [DetectionStatus::KILLED => 'Killed', DetectionStatus::ESCAPED => 'Survived', DetectionStatus::ERROR => 'RuntimeError', DetectionStatus::TIMED_OUT => 'Timeout', DetectionStatus::NOT_COVERED => 'NoCoverage', DetectionStatus::SYNTAX_ERROR => 'CompileError', DetectionStatus::IGNORED => 'Ignored', DetectionStatus::SKIPPED => 'Ignored'];
    ...
    {
        return ['schemaVersion' => '1', 'thresholds' => ['high' => 90, 'low' => 50], 'files' => $this->getFiles(), 'testFiles' => $this->getTestFiles(), 'framework' => ['name' => 'Infection', 'branding' => ['homepageUrl' => 'https://infection.github.io/', 'imageUrl' => 'https://infection.github.io/images/logo.png']]];
    }
    ...
    private function retrieveFiles(array $resultsByPath, string $basePath) : ArrayObject
    {
        ...
        foreach ($resultsByPath as $path => $results) {
            ...
            $files[$relativePath] = ['language' => 'php', 'source' => file_get_contents($path), 'mutants' => $this->retrieveMutants($results, $originalCode)];
        }
        return $files;
    }
    ...
    private function retrieveMutants(array $results, string $originalCode) : array
    {
        return array_map(function (MutantExecutionResult $result) use($originalCode) : array {
            ...
            $methodSignatureMutators = [MutatorFactory::getMutatorNameForClassName(PublicVisibility::class), MutatorFactory::getMutatorNameForClassName(ProtectedVisibility::class)];
            $endingLine = in_array($result->getMutatorName(), $methodSignatureMutators, \true) ? $result->getOriginalStartingLine() : $result->getOriginalEndingLine();
            ...
            return ['id' => $result->getMutantHash(), 'mutatorName' => $result->getMutatorName(), 'replacement' => Str::convertToUtf8(Str::trimLineReturns(ltrim($replacement))), 'description' => $this->getMutatorDescription($result->getMutatorName()), 'location' => ['start' => ['line' => $result->getOriginalStartingLine(), 'column' => $startingColumn], 'end' => ['line' => $endingLine, 'column' => $endingColumn]], 'status' => self::DETECTION_STATUS_MAP[$result->getDetectionStatus()], 'statusReason' => Str::convertToUtf8(Str::trimLineReturns($result->getProcessOutput())), 'coveredBy' => array_unique(array_map(fn(TestLocation $testLocation): string => $this->buildTestMethodId($testLocation->getMethod()), $result->getTests())), 'killedBy' => $this->getKilledBy($result->getProcessOutput()), 'testsCompleted' => $this->getTestsCompleted($result->getProcessOutput())];
        }, $results);
    }
    private function retrieveReplacementFromDiff(string $diff) : string
    {
        $lines = preg_split('/\\n|\\r\\n?/', $diff);
        $lines = array_map(static fn(string $line): string => isset($line[0]) ? substr($line, self::PLUS_LENGTH) : $line, array_filter(array_slice($lines, self::DIFF_HEADERS_LINES_COUNT), static fn(string $line): bool => str_starts_with($line, '+')));
    ...
    {
        return ['id' => $this->buildTestMethodId($testLocation->getMethod()), 'name' => $testLocation->getMethod()];
    }
