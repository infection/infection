...
That memory limit is written by `MemoryLimiter` into the temporary php.ini created by Composer's XdebugHandler, so the scenario deliberately requires active Xdebug and a successful XdebugHandler restart.
