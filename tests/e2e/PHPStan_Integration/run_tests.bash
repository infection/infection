...
then
    echo "PHPStan_Integration requires Xdebug to be installed and loadable with XDEBUG_MODE=coverage." >&2
    exit 1
fi
...
then
    echo "PHPStan_Integration requires Composer XdebugHandler to restart PHP with memory_limit=-1 and a writable inherited php.ini." >&2
    exit 1
fi
