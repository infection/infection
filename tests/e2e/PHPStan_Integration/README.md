...
`phpstan-bootstrap.php` is loaded by PHPStan. It is a no-op for the initial PHPStan run. For mutant PHPStan runs, identified by `--tmp-file=...`, it asserts both parts of the regression setup:
...
Without the fix, mutant PHPStan runs see the effective `32M` limit and error. With the fix, they inherit the capped php.ini but override it with PHP `-d memory_limit=-1`, so the expected summary remains:
