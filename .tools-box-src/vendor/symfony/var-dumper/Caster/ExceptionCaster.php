...
use _HumbugBoxb47773b41c19\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext;
        ...
        if (isset($f['file'], $f['line'])) {
            ...
            if (isset(self::$framesCache[$cacheKey])) {
                ...
                if (\is_file($f['file']) && 0 <= self::$srcContext) {
                    if (!empty($f['class']) && (\is_subclass_of($f['class'], '_HumbugBoxb47773b41c19\\Twig\\Template') || \is_subclass_of($f['class'], '_HumbugBoxb47773b41c19\\Twig_Template')) && \method_exists($f['class'], 'getDebugInfo')) {
                        $template = null;
                        if (isset($f['object'])) {
                            $template = $f['object'];
